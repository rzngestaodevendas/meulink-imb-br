...
function BrokerAuthCard(props: BrokerAuthCardProps) {
  ...
  return <div className="min-h-screen bg-[#f7f8fa] p-4 sm:p-8"><div className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-5xl items-center gap-8 lg:grid-cols-[.9fr_1.1fr]">
    <div className="hidden rounded-3xl bg-[#102c3d] p-10 text-white shadow-xl lg:block"><div className="mb-8 grid h-32 w-32 place-items-center overflow-hidden rounded-3xl bg-white p-3 shadow-2xl ring-4 ring-[#d7b874]/30">{info?.logoUrl ? <img src={info.logoUrl} alt={`Logo ${info.name}`} className="h-full w-full object-contain" /> : <Building2 className="h-12 w-12 text-[#102c3d]" />}</div><p className="text-xs font-bold uppercase tracking-[0.22em] text-[#d7b874]">Portal exclusivo</p><h1 className="mt-4 text-4xl font-semibold tracking-tight">{info?.publicName || info?.name || "Tabela de imóveis"}</h1><p className="mt-4 text-2xl font-semibold leading-tight text-white">Acesse e compartilhe os imóveis com sua assinatura.</p><p className="mt-4 text-sm leading-7 text-white/70">Consulte o estoque disponível, cadastre seus dados profissionais e compartilhe landings com seu nome, foto, CRECI e WhatsApp.</p><div className="mt-8 flex items-center gap-3 text-sm text-white/80"><ShieldCheck className="h-5 w-5 text-[#d7b874]" /> Ambiente profissional para corretores</div></div>
