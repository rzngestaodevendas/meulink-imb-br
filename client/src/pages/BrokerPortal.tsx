...
function BrokerAuthCard(props: BrokerAuthCardProps) {
  ...
  return <div className="min-h-screen bg-[#f3f5f6] p-0 sm:p-5 lg:p-8"><div className="mx-auto grid min-h-screen max-w-7xl overflow-hidden rounded-none bg-white shadow-[0_24px_80px_rgba(16,44,61,0.16)] sm:min-h-[calc(100vh-2.5rem)] sm:rounded-[2rem] lg:grid-cols-[1fr_1fr]">
    ...
    <Card className="min-h-screen rounded-none border-0 bg-white shadow-none sm:min-h-0"><CardHeader className="space-y-5 px-5 pt-7 sm:px-10 sm:pt-10 lg:px-14 lg:pt-14"><div className="flex items-center gap-3"><div className="grid h-16 w-16 place-items-center overflow-hidden rounded-2xl bg-[#102c3d] shadow-lg">{info?.logoUrl ? <img src={info.logoUrl} alt="Logo" className="h-full w-full bg-white object-contain" /> : <Building2 className="h-6 w-6 text-[#d7b874]" />}</div><div><p className="text-xs font-bold uppercase tracking-[0.17em] text-[#b38b3d]">{info?.publicName || info?.name || "MeuLink"}</p><p className="mt-1 text-xs text-slate-500">{info?.developmentName || "Acesso à tabela"}</p></div></div><div><CardTitle className="text-3xl font-semibold tracking-tight text-[#102c3d]">{title}</CardTitle><p className="mt-2 text-sm leading-relaxed text-slate-500">{mode === "register" ? "Informe seus dados completos. Eles serão usados para assinar as landings compartilhadas." : mode === "forgot" ? "Digite seu e-mail. O responsável pela tabela poderá orientar a recuperação do acesso." : "Faça seu cadastro para compartilhar nossos imóveis através de landing pages com o seu perfil profissional."}</p></div>{Boolean(info?.profiles?.length) && <div className="rounded-2xl border border-[#eadfc8] bg-[#fdfaf4] p-3"><p className="mb-3 text-[10px] font-bold uppercase tracking-[0.16em] text-[#b38b3d]">Responsável pela tabela</p><div className="flex gap-3 overflow-x-auto pb-1">{info?.profiles?.map(profile => { const phone = (profile.phone || "").replace(/\D/g, ""); return <div key={profile.name} className="flex min-w-[190px] items-center gap-3 rounded-xl bg-white p-2 shadow-sm"><div className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-full bg-[#102c3d] text-xs font-bold text-white">{profile.photoUrl ? <img src={profile.photoUrl} alt={profile.name} className="h-full w-full object-cover" /> : profile.name.split(/\s+/).slice(0, 2).map(part => part[0]).join("").toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-[#102c3d]">{profile.name}</p>{profile.creci && <p className="truncate text-[10px] text-slate-500">{profile.creci}</p>}{phone && <a href={`https://wa.me/${phone}`} target="_blank" rel="noreferrer" className="mt-1 inline-flex items-center gap-1 text-[10px] font-bold text-[#159447]"><MessageCircle className="h-3 w-3" /> WhatsApp</a>}</div></div>; })}</div></div>}</CardHeader><CardContent>
      {mode === "register" && <div className="mb-4 grid gap-4"><label className="grid gap-2 text-sm font-medium">Nome completo<Input value={fullName} onChange={event => setFullName(event.target.value)} placeholder="Nome e sobrenome" /></label><label className="grid gap-2 text-sm font-medium">Tipo de perfil profissional<select value={profileType} onChange={event => setProfileType(event.target.value as "corretor" | "corretora")} className="h-10 rounded-md border border-slate-200 bg-white px-3 text-sm"><option value="corretor">Corretor</option><option value="corretora">Corretora</option></select></label><div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-2 text-sm font-medium">WhatsApp <span className="text-xs font-normal text-slate-500">Com ou sem o código 55</span><Input value={whatsapp} onChange={event => setWhatsapp(event.target.value.replace(/\D/g, ""))} placeholder="(51) 99999-9999" inputMode="numeric" /></label><label className="grid gap-2 text-sm font-medium">Número CRECI<Input value={creci} onChange={event => setCreci(event.target.value)} placeholder="CRECI 00000-F" /></label></div><label className="grid gap-2 text-sm font-medium">Foto de perfil <span className="text-xs font-normal text-slate-500">JPG, PNG ou WEBP</span><input type="file" accept="image/jpeg,image/png,image/webp" onChange={async event => { const file = event.target.files?.[0]; if (file) await onPhotoUpload(file); }} className="block w-full text-xs text-slate-500 file:mr-3 file:rounded-lg file:border-0 file:bg-[#102c3d] file:px-3 file:py-2 file:text-xs file:font-semibold file:text-white" />{profilePhotoUrl && <span className="text-xs text-emerald-700">Foto carregada com sucesso.</span>}</label></div>}
      ...
      {mode !== "forgot" && <label className="mb-4 grid gap-2 text-sm font-medium">Senha<div className="relative"><Input type={showPassword ? "text" : "password"} value={password} onChange={event => setPassword(event.target.value)} placeholder="Mínimo de 8 caracteres" minLength={8} required className="pr-11" /> <button type="button" onClick={() => setShowPassword(!showPassword)} aria-label={showPassword ? "Ocultar senha" : "Mostrar senha"} className="absolute right-2 top-2 rounded-md p-1.5 text-slate-500 hover:bg-slate-100">{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button></div></label>}
