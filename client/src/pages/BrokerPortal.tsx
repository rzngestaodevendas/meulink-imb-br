...
export default function BrokerPortal() {
  ...
  const groupedByResponsible = Array.from(visibleProperties.reduce((groups, property) => { const key = property.responsibleName?.trim() || "Imóveis sem responsável definido"; const list = groups.get(key) || []; list.push(property); groups.set(key, list); return groups; }, new Map<string, PortalProperty[]>()).entries());
    ...
    <main className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
      <header className="mb-5 overflow-hidden rounded-2xl border border-slate-200 bg-white px-4 py-5 text-[#102c3d] shadow-sm sm:mb-6 sm:rounded-3xl sm:px-8 sm:py-7">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex items-center gap-4"><div className="grid h-16 w-16 shrink-0 sm:h-28 sm:w-28 place-items-center overflow-hidden rounded-2xl border border-slate-100 bg-white p-2 shadow-sm sm:h-28 sm:w-28">{organization?.logoUrl ? <img loading="lazy" decoding="async" src={organization.logoUrl} alt="Logo da construtora" className="h-full w-full object-contain" /> : <Building2 className="h-7 w-7 text-slate-400" />}</div><div><h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">Tabela de imóveis</h1></div></div><div className="flex flex-wrap items-center justify-end gap-2"><div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2"><div className="grid h-11 w-11 place-items-center overflow-hidden rounded-full border-2 border-slate-200 bg-white text-xs font-bold text-[#102c3d]">{user?.profilePhotoUrl ? <img loading="lazy" decoding="async" src={user.profilePhotoUrl} alt={`Foto de ${user.name || "meu perfil"}`} className="h-full w-full object-cover" /> : (user?.name || "C").split(/\s+/).slice(0, 2).map(part => part[0]).join("").toUpperCase()}</div><div className="min-w-0"><p className="max-w-[145px] truncate text-sm font-semibold text-[#102c3d]">{user?.name || "Meu perfil"}</p><button type="button" onClick={() => { setProfileName(user?.name || ""); setProfileTypeEdit(user?.profileType === "corretora" ? "corretora" : "corretor"); setProfileWhatsapp(user?.whatsapp || ""); setProfileCreci(user?.creci || ""); setProfilePhoto(user?.profilePhotoUrl || ""); setEditingProfile(true); }} className="text-[11px] font-semibold text-[#8e6d31] hover:underline">Ver meu perfil</button></div></div><Button type="button" variant="outline" onClick={() => void logout()} className="gap-2 border-slate-200 bg-white text-[#102c3d] hover:bg-slate-50 hover:text-[#102c3d]"><LogOut className="h-4 w-4" /> Sair</Button></div>
        </div>
      </header>

      {editingProfile && <Card className="mb-5 border-0 shadow-sm"><CardContent className="p-4"><div className="mt-5 grid gap-4 border-t border-slate-100 pt-5"><div className="grid gap-4 md:grid-cols-3"><label className="grid gap-2 text-sm font-medium text-slate-700">Nome completo<Input value={profileName} onChange={event => setProfileName(event.target.value)} /></label><label className="grid gap-2 text-sm font-medium text-slate-700">Tratamento no link<select value={profileTypeEdit} onChange={event => setProfileTypeEdit(event.target.value as "corretor" | "corretora")} className="h-10 rounded-md border border-slate-200 bg-white px-3 text-sm"><option value="corretor">Corretor</option><option value="corretora">Corretora</option></select></label><label className="grid gap-2 text-sm font-medium text-slate-700">WhatsApp<Input value={profileWhatsapp} onChange={event => setProfileWhatsapp(event.target.value.replace(/\D/g, ""))} inputMode="numeric" placeholder="5551999999999" /></label><label className="grid gap-2 text-sm font-medium text-slate-700">CRECI<Input value={profileCreci} onChange={event => setProfileCreci(event.target.value)} placeholder="CRECI 00000-F" /></label></div><label className="grid gap-2 text-sm font-medium text-slate-700">Alterar foto de perfil<input type="file" accept="image/jpeg,image/png,image/webp" onChange={async event => { const file = event.target.files?.[0]; if (!file) return; const optimized = await prepareImageForUpload(file); const result = await profileUpload.mutateAsync({ fileName: optimized.fileName, contentType: optimized.contentType, data: optimized.data }); setProfilePhoto(result.url); }} className="block w-full text-xs text-slate-500 file:mr-3 file:rounded-lg file:border-0 file:bg-[#102c3d] file:px-3 file:py-2 file:text-xs file:font-semibold file:text-white" /></label><div className="flex justify-end"><Button onClick={() => updateProfile.mutate({ name: profileName.trim(), profileType: profileTypeEdit, whatsapp: profileWhatsapp.replace(/\D/g, ""), creci: profileCreci.trim(), profilePhotoUrl: profilePhoto })} disabled={updateProfile.isPending || profileUpload.isPending} className="gap-2 bg-[#102c3d] hover:bg-[#173e53]"><Save className="h-4 w-4" /> {updateProfile.isPending ? "Salvando..." : "Salvar meu perfil"}</Button></div></div></CardContent></Card>}
