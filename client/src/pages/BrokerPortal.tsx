...
type BrokerGroup = [string, PortalProperty[]];
...
export default function BrokerPortal() {
  ...
  const groups = useMemo<BrokerGroup[]>(() => {
    const grouped = new Map<string, PortalProperty[]>();
    properties
      .filter(property => `${property.title} ${property.address || ""} ${property.price || ""} ${property.responsibleName || ""}`.toLowerCase().includes(search.toLowerCase()))
      .forEach(property => {
        const name = property.responsibleName?.trim() || "Responsável não informado";
  ...
  return <div className="min-h-screen bg-[#f7f8fa]">
    <main className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
      <header className="mb-6 overflow-hidden rounded-3xl bg-[#102c3d] px-6 py-7 text-white shadow-xl sm:px-8"><div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><div className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-[#d7b874]"><Building2 className="h-4 w-4" /> Portal exclusivo da tabela</div><h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">{organization?.publicName || organization?.name || "Tabela de imóveis"}</h1><p className="mt-2 max-w-2xl text-sm text-white/70">Consulte os imóveis separados por corretor responsável e compartilhe uma apresentação personalizada com seu cliente.</p></div><div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/10 px-4 py-3 text-xs text-white/80"><ShieldCheck className="h-4 w-4 text-[#d7b874]" /> Acesso autenticado</div></div></header>
