    ...
    <main className="mx-auto max-w-7xl p-4 sm:p-6 lg:p-8">
      <header className="mb-6 overflow-hidden rounded-3xl bg-[#102c3d] px-6 py-7 text-white shadow-xl sm:px-8">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex items-center gap-4"><div className="grid h-24 w-24 shrink-0 place-items-center overflow-hidden rounded-2xl border-4 border-white/70 bg-white p-2 shadow-xl sm:h-28 sm:w-28">{organization?.logoUrl ? <img src={organization.logoUrl} alt="Logo da construtora" className="h-full w-full object-contain" /> : <Building2 className="h-7 w-7 text-[#102c3d]" />}</div><div><div className="mb-3 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.2em] text-[#d7b874]"><Building2 className="h-4 w-4" /> {organization?.tableType === "own_development" ? "Empreendimento próprio" : "Imóveis de terceiros"}</div><h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">{organization?.publicName || organization?.name || "Tabela de imóveis"}</h1><p className="mt-2 max-w-2xl text-sm text-white/70">{organization?.developmentName ? `${organization.developmentName} · ` : ""}Consulte o estoque autorizado e compartilhe uma apresentação personalizada com seu cliente.</p></div></div><div className="flex flex-wrap items-center justify-end gap-2"><div className="flex items-center gap-3 rounded-2xl border border-white/15 bg-white/10 px-3 py-2 backdrop-blur"><div className="grid h-11 w-11 place-items-center overflow-hidden rounded-full border-2 border-[#d7b874] bg-[#173e53] text-xs font-bold text-white">{user?.profilePhotoUrl ? <img src={user.profilePhotoUrl} alt={`Foto de ${user.name || "meu perfil"}`} className="h-full w-full object-cover" /> : (user?.name || "C").split(/\s+/).slice(0, 2).map(part => part[0]).join("").toUpperCase()}</div><div className="min-w-0"><p className="max-w-[145px] truncate text-sm font-semibold text-white">{user?.name || "Meu perfil"}</p><button type="button" onClick={() => { setProfileName(user?.name || ""); setProfileTypeEdit(user?.profileType === "corretora" ? "corretora" : "corretor"); setProfileWhatsapp(user?.whatsapp || ""); setProfileCreci(user?.creci || ""); setProfilePhoto(user?.profilePhotoUrl || ""); setEditingProfile(true); }} className="text-[11px] font-semibold text-[#f0d99e] hover:underline">Ver meu perfil</button></div></div><div className="hidden items-center gap-2 rounded-2xl border border-white/10 bg-white/10 px-3 py-3 text-xs text-white/80 md:flex"><ShieldCheck className="h-4 w-4 text-[#d7b874]" /> Acesso autenticado</div><Button type="button" variant="outline" onClick={() => void logout()} className="gap-2 border-white/20 bg-white/10 text-white hover:bg-white/20 hover:text-white"><LogOut className="h-4 w-4" /> Sair</Button></div>
        </div>
      </header>

      <Card className="mb-5 border-0 shadow-sm"><CardContent className="p-4">{editingProfile && <div className="mt-5 grid gap-4 border-t border-slate-100 pt-5"><div className="grid gap-4 md:grid-cols-3"><label className="grid gap-2 text-sm font-medium text-slate-700">Nome completo<Input value={profileName} onChange={event => setProfileName(event.target.value)} /></label><label className="grid gap-2 text-sm font-medium text-slate-700">Tratamento no link<select value={profileTypeEdit} onChange={event => setProfileTypeEdit(event.target.value as "corretor" | "corretora")} className="h-10 rounded-md border border-slate-200 bg-white px-3 text-sm"><option value="corretor">Corretor</option><option value="corretora">Corretora</option></select></label><label className="grid gap-2 text-sm font-medium text-slate-700">WhatsApp<Input value={profileWhatsapp} onChange={event => setProfileWhatsapp(event.target.value.replace(/\D/g, ""))} inputMode="numeric" placeholder="5551999999999" /></label><label className="grid gap-2 text-sm font-medium text-slate-700">CRECI<Input value={profileCreci} onChange={event => setProfileCreci(event.target.value)} placeholder="CRECI 00000-F" /></label></div><label className="grid gap-2 text-sm font-medium text-slate-700">Alterar foto de perfil<input type="file" accept="image/jpeg,image/png,image/webp" onChange={async event => { const file = event.target.files?.[0]; if (!file) return; const data = await new Promise<string>((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result)); reader.onerror = reject; reader.readAsDataURL(file); }); const result = await profileUpload.mutateAsync({ fileName: file.name, contentType: file.type as "image/jpeg" | "image/png" | "image/webp", data }); setProfilePhoto(result.url); }} className="block w-full text-xs text-slate-500 file:mr-3 file:rounded-lg file:border-0 file:bg-[#102c3d] file:px-3 file:py-2 file:text-xs file:font-semibold file:text-white" /></label><div className="flex justify-end"><Button onClick={() => updateProfile.mutate({ name: profileName.trim(), profileType: profileTypeEdit, whatsapp: profileWhatsapp.replace(/\D/g, ""), creci: profileCreci.trim(), profilePhotoUrl: profilePhoto })} disabled={updateProfile.isPending || profileUpload.isPending} className="gap-2 bg-[#102c3d] hover:bg-[#173e53]"><Save className="h-4 w-4" /> {updateProfile.isPending ? "Salvando..." : "Salvar meu perfil"}</Button></div></div>}</CardContent></Card>
...
function BrokerAuthCard(props: BrokerAuthCardProps) {
  ...
  return <div className="min-h-screen bg-[#eef2f4] p-0 sm:p-6 lg:p-10"><div className="mx-auto grid min-h-[calc(100vh-3rem)] max-w-6xl overflow-hidden rounded-[2rem] bg-white shadow-[0_24px_80px_rgba(16,44,61,0.16)] lg:grid-cols-[1fr_1fr]">
    ...
    <Card className="border-0 bg-white shadow-none lg:shadow-none"><CardHeader className="space-y-5 px-2 pt-2 sm:px-4 lg:px-8 lg:pt-8"><div className="flex items-center gap-3"><div className="grid h-16 w-16 place-items-center overflow-hidden rounded-2xl bg-[#102c3d] shadow-lg">{info?.logoUrl ? <img src={info.logoUrl} alt="Logo" className="h-full w-full bg-white object-contain" /> : <Building2 className="h-6 w-6 text-[#d7b874]" />}</div><div><p className="text-xs font-bold uppercase tracking-[0.17em] text-[#b38b3d]">{info?.publicName || info?.name || "MeuLink"}</p><p className="mt-1 text-xs text-slate-500">{info?.developmentName || "Portal do corretor"}</p></div></div><div><CardTitle className="text-3xl font-semibold tracking-tight text-[#102c3d]">{title}</CardTitle><p className="mt-2 text-sm leading-relaxed text-slate-500">{mode === "register" ? "Informe seus dados completos. Eles serão usados para assinar as landings compartilhadas." : mode === "forgot" ? "Digite seu e-mail. O responsável pela tabela poderá orientar a recuperação do acesso." : "Entre para visualizar somente os imóveis disponíveis desta tabela."}</p></div>{Boolean(info?.profiles?.length) && <div className="rounded-2xl border border-[#eadfc8] bg-[#fdfaf4] p-3"><p className="mb-3 text-[10px] font-bold uppercase tracking-[0.16em] text-[#b38b3d]">Corretores responsáveis</p><div className="flex gap-3 overflow-x-auto pb-1">{info?.profiles?.map(profile => { const phone = (profile.phone || "").replace(/\D/g, ""); return <div key={profile.name} className="flex min-w-[190px] items-center gap-3 rounded-xl bg-white p-2 shadow-sm"><div className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-full bg-[#102c3d] text-xs font-bold text-white">{profile.photoUrl ? <img src={profile.photoUrl} alt={profile.name} className="h-full w-full object-cover" /> : profile.name.split(/\s+/).slice(0, 2).map(part => part[0]).join("").toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-xs font-semibold text-[#102c3d]">{profile.name}</p>{profile.creci && <p className="truncate text-[10px] text-slate-500">{profile.creci}</p>}{phone && <a href={`https://wa.me/${phone}`} target="_blank" rel="noreferrer" className="mt-1 inline-flex items-center gap-1 text-[10px] font-bold text-[#159447]"><MessageCircle className="h-3 w-3" /> WhatsApp</a>}</div></div>; })}</div></div>}</CardHeader><CardContent>
