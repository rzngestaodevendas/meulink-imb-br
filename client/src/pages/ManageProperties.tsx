...
function PhotoGallery({ value, onChange, onUpload, uploading, compact = false }: { value: string; onChange: (value: string) => void; onUpload: (files: FileList | File[]) => Promise<void>; uploading: boolean; compact?: boolean }) {
  ...
  const remove = (index: number) => onChange(photos.filter((_, photoIndex) => photoIndex !== index).join("\n"));
...
<PhotoGallery value={form.photos} onChange={photos => setForm({ ...form, photos })} onUpload={addPhotos} uploading={uploadPhoto.isPending} />
