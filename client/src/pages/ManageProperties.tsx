...
function PhotoGallery({ value, onChange, onUpload, uploading }: { value: string; onChange: (value: string) => void; onUpload: (files: FileList | File[]) => Promise<void>; uploading: boolean }) {
  ...
  return <div className="lg:col-span-2 rounded-2xl border border-[#eadfc8] bg-[#fdfaf4] p-4">
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold text-[#102c3d]">Galeria de fotos</p><p className="text-xs font-normal text-slate-500">A primeira foto é a capa. Arraste pelo ícone para reorganizar com mouse ou dedo.</p></div><label className="inline-flex h-10 cursor-pointer items-center justify-center gap-2 rounded-md bg-[#102c3d] px-4 text-xs font-bold text-white hover:bg-[#173e53]"><UploadCloud className="h-4 w-4" /> {uploading ? "Enviando..." : "Escolher fotos do dispositivo"}<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" multiple disabled={uploading || photos.length >= 60} onChange={async event => { await onUpload(event.target.files || []); event.target.value = ""; }} className="sr-only" /></label></div>
