...
import { Building2, Check, Edit3, GripVertical, ImagePlus, Plus, Save, Archive, Star, Trash2, UploadCloud, X, UserRound, UserPlus } from "lucide-react";
...
type BrokerMember = { memberId: number; userId: number; name: string | null; email: string | null; profilePhoto?: string | null; role: string };
...
export default function ManageProperties() {
  ...
  const members = (team.data || []) as BrokerMember[];
  const brokerGroups = useMemo(() => {
    const byName = new Map<string, Property[]>();
    properties.forEach(property => { const name = property.responsibleName?.trim() || "Imóveis sem corretor"; byName.set(name, [...(byName.get(name) || []), property]); });
    members.forEach(member => { const name = member.name?.trim(); if (name && !byName.has(name)) byName.set(name, []); });
    return Array.from(byName.entries()).sort(([a], [b]) => a.localeCompare(b, "pt-BR"));
  }, [members, properties]);

  function openNew(responsibleName = "") { setEditing("new"); setForm({ ...blankForm, responsibleName }); }
  ...
  return <div className="min-h-screen bg-[#f7f8fa]">
    ...
    <div className="space-y-6">{brokerGroups.map(([brokerName, brokerProperties]) => { const member = members.find(item => item.name?.trim() === brokerName); const initials = brokerName.split(/\s+/).filter(Boolean).slice(0, 2).map(part => part[0]).join("").toUpperCase(); return <section key={brokerName} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm"><div className="flex flex-col gap-4 border-b border-slate-100 bg-[#fffdf8] p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-4"><div className="grid h-14 w-14 shrink-0 place-items-center overflow-hidden rounded-full border-2 border-[#d7b874] bg-[#102c3d] text-lg font-bold text-white">{member?.profilePhoto ? <img src={member.profilePhoto} alt={`Foto de ${brokerName}`} className="h-full w-full object-cover" /> : <>{initials || <UserRound className="h-6 w-6" />}</>}</div><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#b38b3d]">Imóveis do corretor</p><h2 className="text-xl font-semibold text-[#102c3d]">{brokerName}</h2><p className="text-xs text-slate-500">{brokerProperties.length} imóveis · {member?.email || "perfil interno"}</p></div></div><div className="flex flex-wrap gap-2"><Button type="button" variant="outline" onClick={() => { window.location.href = "/app/equipe"; }} className="gap-2"><Edit3 className="h-4 w-4" /> Editar perfil</Button><Button type="button" onClick={() => openNew(brokerName === "Imóveis sem corretor" ? "" : brokerName)} className="gap-2 bg-[#102c3d] hover:bg-[#173e53]"><UserPlus className="h-4 w-4" /> Incluir novo imóvel</Button></div></div>{brokerProperties.length > 0 ? <div className="grid gap-4 p-4 md:grid-cols-2 xl:grid-cols-3">{brokerProperties.map(property => <Card key={property.id} className="flex flex-col overflow-hidden border-0 shadow-sm"><div className="relative aspect-[16/9] bg-slate-100">{property.photos[0] ? <img src={property.photos[0]} alt="" className="h-full w-full object-cover" /> : <div className="grid h-full place-items-center text-slate-400">Sem foto</div>}<Badge className="absolute left-3 top-3 bg-white/90 text-[#102c3d]">{statusLabels[property.status]}</Badge></div><CardHeader className="pb-2"><CardTitle className="line-clamp-2 text-base text-[#102c3d]">{property.title}</CardTitle><p className="line-clamp-2 text-xs text-slate-500">{property.address || "Endereço sob consulta"}</p></CardHeader><CardContent className="flex flex-1 flex-col"><p className="mb-3 text-lg font-bold text-[#102c3d]">{property.price || "Valor sob consulta"}</p><p className="mb-4 line-clamp-2 text-xs text-slate-500">{property.details.join(" · ") || "Sem características cadastradas"}</p><div className="mt-auto flex gap-2"><Button variant="outline" onClick={() => openEdit(property)} className="flex-1 gap-2"><Edit3 className="h-4 w-4" /> Editar</Button>{property.status !== "hidden" && <Button variant="outline" onClick={() => { if (window.confirm("Arquivar este imóvel e desativar seus links públicos?")) archive.mutate({ id: property.id }); }} className="gap-2 text-red-700 hover:bg-red-50"><Archive className="h-4 w-4" /> Arquivar</Button>}</div>{property.publicEnabled && property.status === "available" && <p className="mt-3 flex items-center gap-1 text-xs font-medium text-emerald-700"><Check className="h-3 w-3" /> Disponível para links públicos</p>}</CardContent></Card>)}</div> : <div className="flex flex-col items-center gap-3 p-8 text-center text-sm text-slate-500"><p>Nenhum imóvel vinculado a este corretor.</p><Button type="button" variant="outline" onClick={() => openNew(brokerName === "Imóveis sem corretor" ? "" : brokerName)} className="gap-2"><Plus className="h-4 w-4" /> Incluir primeiro imóvel</Button></div>}</section>; })}</div>
