...
export default function Team() {
  ...
  const createInvite = trpc.team.createInvite.useMutation({ onSuccess: result => { const url = `${window.location.origin}/app/equipe?invite=${result.token}`; setCreatedInviteUrl(url); setInviteEmail(""); toast.success("Convite criado"); utils.team.listInvites.invalidate(); }, onError: error => toast.error(error.message) });
  ...
  const acceptInvite = trpc.team.acceptInvite.useMutation({ onSuccess: () => { toast.success("Convite aceito"); window.location.href = "/app"; }, onError: error => toast.error(error.message) });
