...
export default function PublicProperty() {
  ...
  return <main className="min-h-screen bg-[#f4f6f7] text-[#102c3d]">
    ...
    <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 sm:py-8 lg:px-10 lg:py-10">
      ...
      {property.details.length > 0 && <section className="mt-7 rounded-3xl bg-white p-6 shadow-sm sm:p-8"><SectionHeading title="Características" /><div className="grid gap-2 sm:grid-cols-2">{property.details.map(detail => { const isHeading = detail.trim() === detail.trim().toUpperCase() && /[A-ZÁÀÃÂÉÊÍÓÔÕÚÇ]/.test(detail); return isHeading ? <h3 key={detail} className="col-span-full border-b border-slate-100 pb-1 pt-2 text-xs font-extrabold uppercase tracking-[0.14em] text-[#102c3d]">{detail}</h3> : <div key={detail} className="flex items-center gap-2 rounded-xl border border-emerald-100 bg-emerald-50/60 px-3 py-3 text-sm text-slate-600"><CheckCircle2 className="h-4 w-4 shrink-0 text-[#20bd63]" />{detail}</div>; })}</div></section>}

      {galleryPhotos.length > 0 && <section className="mt-7 overflow-hidden rounded-[2rem] bg-white p-2 shadow-sm sm:p-5"><div className="mb-3 flex items-center justify-between gap-3 px-1"><h2 className="text-base font-semibold text-[#102c3d] sm:text-lg">Galeria de fotos</h2><span className="text-xs font-medium text-slate-500">{photoIndex + 1} de {galleryPhotos.length}</span></div><div className="relative aspect-[4/3] min-h-[340px] w-full overflow-hidden rounded-[1.4rem] bg-[#e8edef] sm:aspect-[16/10] sm:min-h-0"><img src={currentPhoto} alt={`Foto ${photoIndex + 2} de ${property.title}`} className="h-full w-full object-contain" />{galleryPhotos.length > 1 && <><button type="button" aria-label="Foto anterior" onClick={() => setPhotoIndex((photoIndex - 1 + galleryPhotos.length) % galleryPhotos.length)} className="absolute left-3 top-1/2 grid h-12 w-12 -translate-y-1/2 place-items-center rounded-full bg-[#102c3d]/80 text-white shadow-lg"><ChevronLeft className="h-5 w-5" /></button><button type="button" aria-label="Próxima foto" onClick={() => setPhotoIndex((photoIndex + 1) % galleryPhotos.length)} className="absolute right-3 top-1/2 grid h-12 w-12 -translate-y-1/2 place-items-center rounded-full bg-[#102c3d]/80 text-white shadow-lg"><ChevronRight className="h-5 w-5" /></button></>}<button type="button" aria-label="Ampliar foto" onClick={() => setLightbox(true)} className="absolute bottom-3 right-3 inline-flex items-center gap-2 rounded-full bg-white/95 px-3 py-2 text-[11px] font-bold text-[#102c3d] shadow-lg"><Expand className="h-3.5 w-3.5" /> Ampliar</button></div><div className="mt-3 flex gap-3 overflow-x-auto pb-1">{galleryPhotos.map((photo, index) => <button type="button" key={photo} onClick={() => setPhotoIndex(index)} className={`h-28 w-44 shrink-0 overflow-hidden rounded-xl border-2 transition sm:h-20 sm:w-32 ${index === photoIndex ? "border-[#d7b874] ring-2 ring-[#d7b874]/40" : "border-transparent opacity-65"}`}><img src={photo} alt={`Miniatura ${index + 2}`} className="h-full w-full object-cover" /></button>)}</div></section>}
