...
export default function PublicProperty() {
  ...
  const signedFriendlyQuery = trpc.catalog.publicFriendlyLink.useQuery({ code, brokerSlug }, { enabled: !token && Boolean(code && brokerSlug), retry: false, staleTime: 60_000, gcTime: 300_000, refetchOnWindowFocus: false });
  const unsignedQuery = trpc.catalog.publicByCode.useQuery({ code }, { enabled: !token && Boolean(code && !brokerSlug), retry: false, staleTime: 60_000, gcTime: 300_000, refetchOnWindowFocus: false });
  const propertyQuery = trpc.catalog.publicLink.useQuery({ token }, { enabled: token.length > 0, retry: false, staleTime: 60_000, gcTime: 300_000, refetchOnWindowFocus: false });
  const activeQuery = token ? propertyQuery : (brokerSlug ? signedFriendlyQuery : unsignedQuery);
  const [photoIndex, setPhotoIndex] = useState(0);
  const [developmentPhotoIndex, setDevelopmentPhotoIndex] = useState(0);
  ...
  return <main className="min-h-screen bg-[#f4f6f7] text-[#102c3d]">
    ...
    <div className="mx-auto max-w-7xl px-4 py-0 sm:px-6 sm:py-8 lg:px-10 lg:py-10">
      <section className="relative -mx-4 min-h-[390px] overflow-hidden rounded-none bg-[#102c3d] shadow-[0_24px_70px_rgba(16,44,61,0.2)] sm:mx-0 sm:min-h-[520px] sm:rounded-[2rem]">
        {coverPhoto && <img loading="eager" fetchPriority="high" decoding="async" src={coverPhoto} alt={`Capa de ${property.title}`} className="absolute inset-0 h-full w-full object-cover" />}
      ...
      {galleryPhotos.length > 0 && <section className="mt-7 overflow-hidden rounded-[2rem] bg-white p-2 shadow-sm sm:p-5"><div className="mb-3 flex items-center justify-between gap-3 px-1"><h2 className="text-base font-semibold text-[#102c3d] sm:text-lg">Galeria de fotos</h2><span className="text-xs font-medium text-slate-500">{photoIndex + 1} de {galleryPhotos.length}</span></div><div className="relative w-full overflow-hidden rounded-[1.4rem] bg-[#e8edef]"><button type="button" aria-label="Ampliar foto do imóvel" onClick={() => { setLightboxGallery("property"); setZoom(1); setLightbox(true); }} className="block w-full"><div className="flex aspect-[4/3] min-h-[300px] w-full items-center justify-center sm:aspect-[16/8] sm:min-h-[420px]"><img loading="lazy" decoding="async" src={currentPhoto} alt={`Foto ${photoIndex + 2} de ${property.title}`} className="h-full w-full object-cover" /></div></button>{galleryPhotos.length > 1 && <><button type="button" aria-label="Foto anterior do imóvel" onClick={() => { setPhotoIndex(index => (index - 1 + galleryPhotos.length) % galleryPhotos.length); setZoom(1); }} className="absolute left-3 top-1/2 grid h-12 w-12 -translate-y-1/2 place-items-center rounded-full bg-[#102c3d]/80 text-white shadow-lg transition hover:bg-[#102c3d]"><ChevronLeft className="h-5 w-5" /></button><button type="button" aria-label="Próxima foto do imóvel" onClick={() => { setPhotoIndex(index => (index + 1) % galleryPhotos.length); setZoom(1); }} className="absolute right-3 top-1/2 grid h-12 w-12 -translate-y-1/2 place-items-center rounded-full bg-[#102c3d]/80 text-white shadow-lg transition hover:bg-[#102c3d]"><ChevronRight className="h-5 w-5" /></button></>}<button type="button" aria-label="Abrir foto do imóvel" onClick={() => { setLightboxGallery("property"); setZoom(1); setLightbox(true); }} className="absolute bottom-3 right-3 rounded-full bg-white/95 px-3 py-2 text-[11px] font-bold text-[#102c3d] shadow-lg"><Expand className="mr-1 inline h-3.5 w-3.5" /> Ampliar</button></div></section>}
      ...
      {developmentPhotos.length > 0 && <section className="mt-7 overflow-hidden rounded-[2rem] bg-white p-2 shadow-sm sm:p-5"><div className="mb-3 flex items-center justify-between gap-3 px-1"><h2 className="text-base font-semibold text-[#102c3d] sm:text-lg">Fotos do empreendimento</h2><span className="text-xs font-medium text-slate-500">{developmentPhotoIndex + 1} de {developmentPhotos.length}</span></div><div className="relative w-full overflow-hidden rounded-[1.4rem] bg-[#e8edef]"><div className="flex aspect-[4/3] min-h-[300px] w-full items-center justify-center sm:aspect-[16/8] sm:min-h-[420px]"><img loading="lazy" decoding="async" src={developmentPhotos[developmentPhotoIndex]} alt={`Foto ${developmentPhotoIndex + 1} do empreendimento`} className="h-full w-full object-cover" /></div>{developmentPhotos.length > 1 && <><button type="button" aria-label="Foto anterior do empreendimento" onClick={() => setDevelopmentPhotoIndex(index => (index - 1 + developmentPhotos.length) % developmentPhotos.length)} className="absolute left-3 top-1/2 grid h-12 w-12 -translate-y-1/2 place-items-center rounded-full bg-[#102c3d]/80 text-white shadow-lg transition hover:bg-[#102c3d]"><ChevronLeft className="h-5 w-5" /></button><button type="button" aria-label="Próxima foto do empreendimento" onClick={() => setDevelopmentPhotoIndex(index => (index + 1) % developmentPhotos.length)} className="absolute right-3 top-1/2 grid h-12 w-12 -translate-y-1/2 place-items-center rounded-full bg-[#102c3d]/80 text-white shadow-lg transition hover:bg-[#102c3d]"><ChevronRight className="h-5 w-5" /></button></>}<button type="button" aria-label="Abrir foto do empreendimento" onClick={() => { setLightboxGallery("development"); setZoom(1); setLightbox(true); }} className="absolute bottom-3 right-3 rounded-full bg-white/95 px-3 py-2 text-[11px] font-bold text-[#102c3d] shadow-lg">Ampliar</button></div></section>}
      ...
      {broker && <section className="mt-7 rounded-3xl bg-white p-6 shadow-sm sm:p-8"><SectionHeading title="Dados do responsável" /><div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-4"><div className="grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-2xl border-4 border-[#d7b874] bg-[#102c3d] text-xl font-extrabold text-white">{broker.photoUrl ? <img loading="lazy" decoding="async" src={broker.photoUrl} alt={`Foto de ${broker.name}`} className="h-full w-full object-cover" /> : broker.name.split(/\s+/).slice(0, 2).map(part => part[0]).join("").toUpperCase()}</div><div><p className="text-lg font-bold text-[#102c3d]">{broker.name}</p><p className="mt-1 text-xs text-slate-500">Atendimento direto pelo WhatsApp</p></div></div>{whatsapp && <a href={whatsapp} target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-[#20bd63] px-5 text-sm font-extrabold text-white transition hover:bg-[#12934a]"><MessageCircle className="h-5 w-5" /> WhatsApp</a>}</div><div className="mt-5 rounded-2xl border border-emerald-100 bg-emerald-50/70 p-4 text-xs leading-relaxed text-slate-600">Solicite mais informações ou agende sua visita!</div></section>}
    </div>
    {lightbox && lightboxPhoto && <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-[#06141d]/95 p-4 sm:p-8" role="dialog" aria-modal="true" onWheel={event => { event.preventDefault(); setZoom(value => Math.min(3, Math.max(1, value + (event.deltaY < 0 ? 0.25 : -0.25)))); }}><button type="button" aria-label="Fechar galeria" onClick={() => setLightbox(false)} className="absolute right-5 top-5 z-20 rounded-full border border-white/20 bg-white/10 p-2 text-white"><X className="h-5 w-5" /></button><div className="flex min-h-0 flex-1 w-full items-center justify-center overflow-auto py-14"><img src={lightboxPhoto} alt={`Foto ${lightboxIndex + 1}`} style={{ transform: `scale(${zoom})` }} className="max-h-full max-w-full origin-center object-contain transition-transform duration-200" /></div>{lightboxPhotos.length > 1 && <><button type="button" aria-label="Foto anterior ampliada" onClick={() => { if (lightboxGallery === "development") setDevelopmentPhotoIndex(index => (index - 1 + developmentPhotos.length) % developmentPhotos.length); else setPhotoIndex(index => (index - 1 + galleryPhotos.length) % galleryPhotos.length); setZoom(1); }} className="absolute left-3 top-1/2 z-20 grid h-12 w-12 -translate-y-1/2 place-items-center rounded-full bg-white/15 text-white shadow-lg transition hover:bg-white/25 sm:left-6"><ChevronLeft className="h-6 w-6" /></button><button type="button" aria-label="Próxima foto ampliada" onClick={() => { if (lightboxGallery === "development") setDevelopmentPhotoIndex(index => (index + 1) % developmentPhotos.length); else setPhotoIndex(index => (index + 1) % galleryPhotos.length); setZoom(1); }} className="absolute right-3 top-1/2 z-20 grid h-12 w-12 -translate-y-1/2 place-items-center rounded-full bg-white/15 text-white shadow-lg transition hover:bg-white/25 sm:right-6"><ChevronRight className="h-6 w-6" /></button></>}<div className="absolute bottom-5 left-1/2 z-20 flex -translate-x-1/2 items-center gap-2 rounded-2xl border border-white/15 bg-[#102c3d]/90 p-2 text-white shadow-2xl"><button type="button" aria-label="Diminuir zoom" onClick={() => setZoom(value => Math.max(1, value - 0.25))} disabled={zoom <= 1} className="grid h-10 w-10 place-items-center rounded-xl bg-white/10 text-xl font-bold transition hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-40">−</button><span className="min-w-16 text-center text-xs font-bold">{Math.round(zoom * 100)}%</span><button type="button" aria-label="Aumentar zoom" onClick={() => setZoom(value => Math.min(3, value + 0.25))} disabled={zoom >= 3} className="grid h-10 w-10 place-items-center rounded-xl bg-white/10 text-xl font-bold transition hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-40">+</button><button type="button" aria-label="Redefinir zoom" onClick={() => setZoom(1)} className="rounded-xl bg-white/10 px-3 py-2 text-xs font-bold transition hover:bg-white/20">Redefinir</button></div></div>}
  </main>;
}
