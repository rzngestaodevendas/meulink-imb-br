  ...
  return <div className="min-h-screen bg-[#f5f7f8] text-[#102c3d]">
    <main className="mx-auto max-w-[1440px] px-4 py-5 sm:px-6 sm:py-8 lg:px-10 lg:py-10">
      <header className={`relative mb-8 overflow-hidden rounded-2xl border shadow-[0_18px_50px_rgba(16,44,61,0.08)] sm:rounded-[2rem] ${isMasterplan ? "border-black bg-black" : "border-slate-200 bg-white"}`}>
        <div className={`relative flex min-h-56 items-center justify-center overflow-hidden px-3 py-5 sm:min-h-72 sm:px-8 sm:py-8 lg:min-h-80 ${isMasterplan ? "bg-black" : "bg-white"}`}>{organization.coverPhotoUrl ? <img loading="eager" fetchPriority="high" decoding="async" src={mediaUrl(organization.coverPhotoUrl, 1600)} alt={`Capa de ${organizationName}`} className="absolute inset-0 h-full w-full object-cover" /> : null}<div className={`absolute inset-0 ${isMasterplan ? "bg-gradient-to-t from-black/45 via-black/10 to-transparent" : "bg-transparent"}`} />{organization.logoUrl ? <div className={`relative z-10 flex w-full max-w-4xl items-center justify-center ${isMasterplan ? "bg-transparent" : "bg-white px-4 py-3 sm:px-6 sm:py-4"}`}><img loading="eager" fetchPriority="high" decoding="async" src={mediaUrl(organization.logoUrl, 960)} alt={`Logo ${organizationName}`} className="max-h-40 w-full object-contain sm:max-h-52 lg:max-h-60" /></div> : <Building2 className="relative z-10 h-16 w-16 text-slate-400" />}</div>
      </header>
