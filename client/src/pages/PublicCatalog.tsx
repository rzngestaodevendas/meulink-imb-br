...
export default function PublicCatalog() {
  ...
  const shareOrganizationName = organizationName.toLowerCase().includes("atlântida negócios") ? "Atlântida Negócios Imobiliários" : organizationName;
  const profileShareTitle = (name: string) => `Tabela de imóveis - ${name} | ${shareOrganizationName}`;
  ...
  async function sharePublicLink(url: string, title: string) {
    if (navigator.share) {
      try { await navigator.share({ title, text: title, url }); return; } catch (error) { if ((error as DOMException).name === "AbortError") return; }
    }
