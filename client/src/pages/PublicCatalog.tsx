...
import { Building2, CheckCircle2, Link2, LogIn, MapPin, MessageCircle, Share2 } from "lucide-react";
...
function OptionalDetails({ property }: { property: Property }) {
  const values = [
    ...
    property.bedrooms !== null && property.bedrooms !== undefined && ["Dormitórios", String(property.bedrooms)],
    property.suites !== null && property.suites !== undefined && ["Suítes", String(property.suites)],
    property.bathrooms !== null && property.bathrooms !== undefined && ["Banheiros", String(property.bathrooms)],
    property.unitNumber && ["Unidade", property.unitNumber],
    property.privateArea && ["Área privativa", property.privateArea],
    property.garageSpaces !== null && property.garageSpaces !== undefined && ["Vagas", String(property.garageSpaces)],
...
export default function PublicCatalog() {
  ...
  async function sharePublicLink(url: string, title: string) {
    if (navigator.share) {
      try { await navigator.share({ title, text: `Confira este imóvel: ${title}`, url }); return; } catch (error) { if ((error as DOMException).name === "AbortError") return; }
    }
  ...
  return <div className="min-h-screen bg-[#f5f7f8] text-[#102c3d]">
    <main className="mx-auto max-w-[1440px] px-4 py-5 sm:px-6 sm:py-8 lg:px-10 lg:py-10">
      <header className={`relative mb-8 overflow-hidden rounded-2xl border shadow-[0_18px_50px_rgba(16,44,61,0.08)] sm:rounded-[2rem] ${isMasterplan ? "border-black bg-black" : "border-slate-200 bg-white"}`}>
        <div className={`relative flex min-h-56 items-center justify-center overflow-hidden px-3 py-5 sm:min-h-72 sm:px-8 sm:py-8 lg:min-h-80 ${isMasterplan ? "bg-black" : "bg-white"}`}><div className="relative flex h-full w-full items-center justify-center">{organization.logoUrl ? <img src={organization.logoUrl} alt={`Logo ${organizationName}`} className="max-h-40 w-full max-w-4xl object-contain sm:max-h-52 lg:max-h-60" /> : <Building2 className="h-16 w-16 text-slate-400" />}</div></div>
      </header>
      ...
      <section className="mb-10 rounded-3xl border border-[#dce8df] bg-white p-5 shadow-sm sm:p-7"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#b38b3d]">Compartilhamento profissional</p><p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-600">Compartilhe com seus clientes através de uma página personalizada com a sua identidade profissional, com seu nome, foto, Creci e WhatsApp.</p></div><a href={`${publicBase}?acesso=login`} className="inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl bg-[#20bd63] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#12934a]"><LogIn className="h-4 w-4" /> Entrar na área do corretor</a></div></section>
      <section className="space-y-10">
        {Array.from(groups.values()).map(group => {
          ...
          return <section key={group.name} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_8px_30px_rgba(16,44,61,0.08)]"><div className="flex flex-col gap-4 border-b border-slate-100 bg-[#fffdf8] p-5 sm:flex-row sm:items-center sm:justify-between sm:p-7"><div className="flex items-center gap-4"><div className="grid h-24 w-24 shrink-0 place-items-center overflow-hidden rounded-2xl bg-[#102c3d] text-2xl font-bold text-white shadow-md sm:h-28 sm:w-28">{group.profile?.photoUrl ? <img src={group.profile.photoUrl} alt={group.name} className="h-full w-full object-cover" /> : initials(group.name)}</div><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#b38b3d]">Responsável</p><h2 className="mt-1 text-2xl font-semibold text-[#102c3d]">{group.name}</h2>{group.profile?.creci && <p className="mt-1 text-sm text-slate-500">{group.profile.creci}</p>}</div></div><div className="flex flex-wrap gap-2">{groupPhone && <a href={`https://wa.me/${groupPhone}?text=${encodeURIComponent(`Olá ${group.name}, gostaria de informações sobre os imóveis.`)}`} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#20bd63] px-5 py-3 text-sm font-bold text-white transition hover:bg-[#12934a]"><MessageCircle className="h-5 w-5" /> WhatsApp</a>}<a href={profileUrl} className="inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-200 px-5 py-3 text-sm font-semibold text-[#102c3d] hover:border-[#d7b874]">Ver página</a></div></div>{group.properties.length ? <div className="grid gap-4 p-4 sm:grid-cols-2 xl:grid-cols-4">{group.properties.map(property => { const phone = phoneForProperty(property); const unsignedUrl = `${window.location.origin}/imovel/${property.code}`; const signedUrl = `${publicBase}?compartilhar=${property.id}`; return <Card key={property.id} className="group flex h-full flex-col overflow-hidden rounded-2xl border-0 bg-white shadow-[0_8px_24px_rgba(16,44,61,0.07)]"><div className="relative aspect-[4/3] overflow-hidden bg-slate-100">{property.photos[0] ? <img src={property.photos[0]} alt={property.title} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="grid h-full place-items-center text-sm text-slate-400">Sem foto</div>}<Badge className="absolute left-3 top-3 border-0 bg-white/95 text-[#102c3d]">{property.code}</Badge></div><CardHeader className="pb-2"><CardTitle className="line-clamp-2 text-base leading-snug text-[#102c3d]">{property.title}</CardTitle>{property.developmentName && <p className="mt-1 line-clamp-1 text-xs font-semibold text-[#b38b3d]">{property.developmentName}</p>}<p className="mt-1 flex items-start gap-1 text-xs text-slate-500"><MapPin className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[#b38b3d]" /><span className="line-clamp-2">{property.address || "Endereço sob consulta"}</span></p></CardHeader><CardContent className="mt-auto"><p className="text-lg font-bold text-[#102c3d]">{property.price || "Valor sob consulta"}</p><OptionalDetails property={property} /><div className="mt-5 grid gap-2">{phone && <a href={`https://wa.me/${phone}?text=${encodeURIComponent(`Olá ${nameForProperty(property)}, gostaria de informações sobre o imóvel ${property.code} - ${property.title}.`)}`} target="_blank" rel="noreferrer" className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg bg-[#20bd63] px-3 py-2 text-center text-xs font-bold text-white hover:bg-[#12934a]"><MessageCircle className="h-4 w-4" /> Mais informações</a>}<a href={signedUrl} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg bg-[#102c3d] px-3 py-2 text-center text-xs font-bold text-white hover:bg-[#173e53]"><LogIn className="h-4 w-4" /> Entrar e compartilhar com seus dados</a><button type="button" onClick={() => sharePublicLink(unsignedUrl, property.title)} className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-center text-xs font-semibold text-[#102c3d] hover:border-[#d7b874]"><Share2 className="h-4 w-4" /> Compartilhar sem seus dados</button></div></CardContent></Card>; })}</div> : <div className="p-8 text-sm text-slate-500">Nenhum imóvel disponível para este responsável.</div>}</section>;
        })}
