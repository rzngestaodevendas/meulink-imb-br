...
import { invalidateOrganizationPublicCache } from "./publicCache";
...
async function requireCompanyAdminForOrganization(ctx: { user: { id: number; openId: string; role: string; activeOrganizationId?: number | null } }, organizationId: number) {
  await ensureOrganizationColumns();
  ...
  if (ctx.user.role === "admin" || ctx.user.openId === ENV.ownerOpenId) {
    ...
    return { organizationId, memberRole: "company_admin" as const, organization };
  }
  const scope = await getOrganizationForUser(ctx.user.id, ctx.user.openId, ctx.user.role, organizationId);
  if (!scope) throw new TRPCError({ code: "FORBIDDEN", message: "Você não pertence a esta tabela." });
  if (scope.memberRole !== "company_admin") throw new TRPCError({ code: "FORBIDDEN", message: "Somente administradores da empresa podem gerenciar esta tabela." });
  return scope;
}
...
export const appRouter = router({
  ...
  developments: router({
    ...
    create: protectedProcedure.input(z.object({ name: z.string().trim().min(2).max(240), description: z.string().max(10000).default(""), photos: z.array(z.string().trim().min(1).max(1000)).max(60).default([]), details: z.array(z.string().trim().min(1).max(240)).max(60).default([]), mapUrl: z.string().trim().url().or(z.literal("")).default(""), mapDriveUrl: z.string().trim().url().or(z.literal("")).default(""), photosDriveUrl: z.string().trim().url().or(z.literal("")).default(""), videosDriveUrl: z.string().trim().url().or(z.literal("")).default("") })).mutation(async ({ ctx, input }) => {
      ...
      await invalidateOrganizationPublicCache(db, scope.organizationId);
    ...
    delete: protectedProcedure.input(z.object({ id: z.number().int().positive() })).mutation(async ({ ctx, input }) => {
      ...
      await db.update(properties).set({ developmentId: null }).where(and(eq(properties.organizationId, scope.organizationId), eq(properties.developmentId, input.id))); await db.delete(developments).where(eq(developments.id, input.id)); await invalidateOrganizationPublicCache(db, scope.organizationId); return { success: true as const };
    }),
  }),
  ...
  organizations: router({
    list: protectedProcedure.query(async ({ ctx }) => {
      ...
      return Promise.all(rows.map(async organization => {
        ...
        return { ...organization, slug: publicOrganizationSlug(organization.slug), availablePropertyCount: availableProperties.length, availableProperties: availableProperties.map(property => parseProperty(property)) };
      }));
    }),
    ...
    linkProperties: protectedProcedure.input(z.object({ organizationId: z.number().int().positive(), propertyIds: z.array(z.number().int().positive()).min(1).max(200), responsibleName: z.string().trim().max(180).optional(), responsiblePhone: z.string().trim().max(32).optional() })).mutation(async ({ ctx, input }) => {
      ...
      await invalidateOrganizationPublicCache(db, input.organizationId);
  ...
  responsibleProfiles: router({
    list: protectedProcedure.input(z.object({ organizationId: z.number().int().positive() })).query(async ({ ctx, input }) => {
      ...
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Banco indisponível" });
      const scope = await requireCompanyAdminForOrganization(ctx, input.organizationId);
  ...
  catalog: router({
    list: protectedProcedure.input(propertyInput.extend({ organizationId: z.number().int().positive().optional() }).optional()).query(async ({ ctx, input }) => {
      await ensurePropertyPrivateColumns();
      const db = await getDb();
      if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Banco indisponível" });
      const scope = input?.organizationId ? await requireCompanyAdminForOrganization(ctx, input.organizationId) : await requireCompanyAdmin(ctx);
